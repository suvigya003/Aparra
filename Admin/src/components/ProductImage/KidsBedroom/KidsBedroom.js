...
const KidsBedroom = () => {
  ...
  useEffect(() => {
    const getModularKitchenTableData = async () => {
      try{
        const {data} = await axios.get('http://localhost:8080/api/aparra/getKidsBedroom');
  ...
  const handleSubmit = async(e) => {
    ...
    try{
      ...
      await axios.post(`http://localhost:8080/api/aparra/addKidsBedroom`, formData).then((res) => {
        console.log(res);
      }
      ).catch((err) => {
        console.log(err);
