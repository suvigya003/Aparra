...
const ProductImageTabs = () => {
  ...
  useEffect(() => {
    const getModularKitchenTableData = async () => {
      try{
        const {data} = await axios.get('http://localhost:8080/api/aparra/getStudyTable');
  ...
  const handleSubmit = async(e) => {
    ...
    try{
      ...
      await axios.post(`http://localhost:8080/api/aparra/addStudyTable`, formData).then((res) => {
        console.log(res);
      }
      ).catch((err) => {
        console.log(err);
