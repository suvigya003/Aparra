...
const StorageWardrobe = () => {
  ...
  useEffect(() => {
    const getModularKitchenTableData = async () => {
      try{
        const {data} = await axios.get('http://localhost:8080/api/aparra/getStorageWardrobe');
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
    try{
      console.log('modularKitchen', modularKitchen);
      console.log('image', image);
      const formData = new FormData();
      formData.append('image', image);
      formData.append('desc', modularKitchen.desc);
      formData.append('size', modularKitchen.size);
      console.log('formData', formData);

      await axios.post(`http://localhost:8080/api/aparra/addStorageWardrobe`, formData).then((res) => {
        console.log(res);
      }
      ).catch((err) => {
        console.log(err);
          ...
          <Scrollbar>
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                ...
                />
                <TableBody>
                  {modularKitchenTable.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
...
export default StorageWardrobe;
