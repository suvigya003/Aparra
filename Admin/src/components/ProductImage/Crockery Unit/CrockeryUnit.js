...
const CrockeryUnit = () => {
  ...
  useEffect(() => {
    const getModularKitchenTableData = async () => {
      try{
        const {data} = await axios.get('http://localhost:8080/api/aparra/getCrockeryUnit');
  ...
  const handleSubmit = async(e) => {
    ...
    try{
      ...
      await axios.post(`http://localhost:8080/api/aparra/addCrockeryUnit`, formData).then((res) => {
        console.log(res);
      }
      ).catch((err) => {
        console.log(err);
